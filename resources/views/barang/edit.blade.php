...
<div class="animate-fade-in max-w-3xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4 py-2">
        ...
        <div>
            ...
            <p class="text-slate-400 text-xs mt-1">
                Perbarui rincian informasi produk di bawah ini.
            </p>
        </div>
    </div>
    ...
    <div class="glass-card p-6 sm:p-8 rounded-3xl">
        <form action="{{ route('uas.barang.update', $barang) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="kode_barang" id="kode_barang" value="{{ old('kode_barang', $barang->kode_barang) }}" required
                           class="w-full px-4 py-2.5 bg-slate-900 border border-slate-700/80 rounded-xl text-slate-200 focus:outline-none focus:border-sky-500 transition-colors text-sm @error('kode_barang') border-rose-500 @enderror">
                    @error('kode_barang')
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="nama_barang" id="nama_barang" value="{{ old('nama_barang', $barang->nama_barang) }}" required
                           class="w-full px-4 py-2.5 bg-slate-900 border border-slate-700/80 rounded-xl text-slate-200 focus:outline-none focus:border-sky-500 transition-colors text-sm @error('nama_barang') border-rose-500 @enderror">
                    @error('nama_barang')
                        <p class="text-xs text-rose-400 mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="kategori" id="kategori" value="{{ old('kategori', $barang->kategori) }}" required
                           class="w-full px-4 py-2.5 bg-slate-900 border border-slate-700/80 rounded-xl text-slate-200 focus:outline-none focus:border-sky-500 transition-colors text-sm @error('kategori') border-rose-500 @enderror">
                    @error('kategori')
                        <p class="text-xs text-rose-400 mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="harga" id="harga" value="{{ old('harga', $barang->harga) }}" required min="0"
                               class="w-full pl-10 pr-4 py-2.5 bg-slate-900 border border-slate-700/80 rounded-xl text-slate-200 focus:outline-none focus:border-sky-500 transition-colors text-sm @error('harga') border-rose-500 @enderror">
                    </div>
                    @error('harga')
                        <p class="text-xs text-rose-400 mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="number" name="stok" id="stok" value="{{ old('stok', $barang->stok) }}" required min="0"
                ...
                <div class="space-y-1.5 flex flex-col justify-end">
                    <label for="gambar" class="text-xs text-slate-450 font-bold uppercase tracking-wider block">Ganti Gambar Produk</label>
                    
                    <div class="flex items-center gap-4 mt-1">
                        <!-- Current Thumbnail -->
                        <div class="h-12 w-12 rounded-xl bg-slate-900 border border-slate-800 overflow-hidden flex-shrink-0 flex items-center justify-center">
                            @if($barang->gambar)
                                <img src="{{ asset('storage/' . $barang->gambar) }}" alt="" class="h-full w-full object-cover">
                            @else
                                <i class="fa-solid fa-image text-slate-700"></i>
                            @endif
                        </div>
                        
                        <div class="flex-grow">
                            <input type="file" name="gambar" id="gambar" accept="image/*"
                                   class="w-full px-3 py-1.5 bg-slate-900 border border-slate-700/80 rounded-xl text-slate-400 text-xs focus:outline-none focus:border-sky-500 file:mr-3 file:py-1 file:px-2.5 file:rounded-lg file:border-0 file:text-[10px] file:font-semibold file:bg-slate-800 file:text-slate-300 hover:file:bg-slate-700 file:cursor-pointer @error('gambar') border-rose-500 @enderror">
                        </div>
                    </div>
                    <p class="text-[9px] text-slate-500">Kosongkan jika tidak ingin mengubah gambar. Maks 2MB.</p>
                    @error('gambar')
                        <p class="text-xs text-rose-400 mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="sm:col-span-2 space-y-1.5">
                    ...
                    <textarea name="deskripsi" id="deskripsi" rows="4" placeholder="Tuliskan deskripsi lengkap mengenai detail produk..."
                              class="w-full px-4 py-2.5 bg-slate-900 border border-slate-700/80 rounded-xl text-slate-200 placeholder-slate-500 focus:outline-none focus:border-sky-500 transition-colors text-sm @error('deskripsi') border-rose-500 @enderror">{{ old('deskripsi', $barang->deskripsi) }}</textarea>
                    @error('deskripsi')
                        <p class="text-xs text-rose-400 mt-1">{{ $message }}</p>
                    @enderror
                </div>
